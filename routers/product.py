...
def create_product(product: ProductCreate, conn: MySQLConnection = Depends(get_db_connection)):
    return create_product_db(product,conn)

@router.get("/", response_model=list[ProductRead] )
def get_products( conn: MySQLConnection = Depends(get_db_connection)):
    return get_all_products_db(conn)

@router.get("/{product_id}", response_model=ProductRead)
def get_product(product_id: int, conn: MySQLConnection = Depends(get_db_connection)):
