...
def create_supplier(supplier: SupplierCreate,conn: MySQLConnection = Depends(get_db_connection)):
    return create_supplier_db(supplier, conn)

@router.get("/", response_model=list[SupplierRead])
def get_suppliers( conn: MySQLConnection = Depends(get_db_connection)):
    return get_all_suppliers_db(conn)

@router.get("/{supplier_id}", response_model=SupplierRead)
def get_supplier(supplier_id: int, conn: MySQLConnection = Depends(get_db_connection)):
    return get_supplier_by_id_db(supplier_id, conn)

@router.delete("/{supplier_id}")
def delete_supplier(supplier_id: int,conn: MySQLConnection = Depends(get_db_connection)):
