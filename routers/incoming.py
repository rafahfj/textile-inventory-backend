...
def create_incoming(data: IncomingCreate, conn: MySQLConnection = Depends(get_db_connection)):
    return create_incoming_db(data, conn)

@router.get("/", response_model=list[IncomingRead])
def get_all_incoming( conn: MySQLConnection = Depends(get_db_connection)):
    return get_all_incoming_db(conn)

@router.get("/{id}", response_model=IncomingRead)
def get_incoming(id: int, conn: MySQLConnection = Depends(get_db_connection)):
    return get_incoming_by_id_db(id, conn)

@router.delete("/{id}")
def delete_incoming(id: int, conn: MySQLConnection = Depends(get_db_connection)):
